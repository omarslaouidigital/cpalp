...
const Es = () => {
        ...
        if(undefined !== window){
            ...
            if(params){
                ...
                json_data.forEach(elt => {
                    console.log(elt.id)
                    if(elt.id == id){
                        setTitle(elt.es_title)
                        setLocker(elt.es_locker)
                        setDescription(elt.es_description)
    ...
    useEffect(() => {
        if(loading){
            ...
            setProgressText('descargando ' + title + '...')
            setTimeout(() => {
                setProgressText('Desembalaje ' + title + '.gz')
            }, 8000)
            setTimeout(() => {
                setProgressText('Cargando la aplicación en su dispositivo...')
            }, 16000)
    ...
    return (
        ...
        <div className="bg-slate-100 w-full min-h-screen overflow-x-hidden">
            ...
            {/* H T M L   H E A D */}
            <Head>
                <title>{lp_name} - Aplicaciones Tweaked y Premium gratis, Jailbreak y muchas más...</title>
            ...
            <div className="w-full shadow-md text-center text-white bg-red-500 py-4 px-5">
                ...
                <p className="animate__animated animate__backInLeft text-sm">Aplicaciones Tweaked y Premium gratis, Jailbreak y muchas más...</p>
            </div>
            ...
            {step_2 && 
                <>
                    ...
                    <div className="w-5/6 md:w-2/6 bg-gray-200 rounded-xl shadow-lg py-8 text-center fixed top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 z-50 animate__animated animate__fadeIn">
                        ...
                        {!loading ?
                            <>
                            <h1 className="text-red-500 font-bold text-xl">Descargar listo</h1>
                            <p className="px-6 text-gray-600">Haga clic en el botón a continuación para comenzar con la descarga de su aplicación.</p>
                            <button onClick={e => setLoading(true)} className="mt-4 py-4 px-8 bg-red-500 rounded-xl shadow-lg font-bold text-white animate__animated animate__pulse animate__infinite">DESCARGAR AHORA</button>
                            </>
                            :
                            <> 
            ...
            <div className="w-5/6 mx-auto my-4">
                ...
                <div className="relative text-center md:w-2/6 mx-auto animate__animated animate__pulse animate__infinite">
                    ...
                    <input onChange={e => setSearch(e.target.value)} type="text" className="w-full mx-auto bg-gray-300 rounded-xl shadow-sm py-3 text-gray-200 text-lg text-center" placeholder="Buscar aplicaciones..."/>
                </div>
                ...
                <div className="grid gap-3 grid-cols-1 lg:grid-cols-2 lg:mt-2 lg:w-3/6 lg:mx-auto">
                    ...
                    {niches.map((niche, i) => (
                        niche.es_title &&
                        <div key={i} onClick={e => choose_niche(e)} className={`${search !== '' && !niche.es_keywords.includes(search.toLowerCase()) && 'hidden'} w-full bg-gray-50 rounded-xl shadow-lg mt-4 md:my-2 relative py-8 cursor-pointer `}>
                            <div className="locker hidden">{niche.es_locker}</div>
                            ...
                            <div className='flex gap-3 px-3'>
                                ...
                                <div>
                                    <h6 className='text-gray-300 text-sm'>Autor: <span className="text-red-300 author">{niche.author}</span></h6>
                                    <h1 className="font-bold text-gray-600 text-2xl">{niche.es_title}</h1>
                                    <p className="text-gray-400 text-sm description">{niche.es_description}</p>
                                    <div className='flex'>
        ...
        <div className="bg-gray-100 w-full min-h-screen overflow-x-hidden">
            ...
            {/* H T M L   H E A D */}
            <Head>
                <title>Aplicativos pagos gratuitos, jogos hackeados e muito mais {lp_name}</title>
                <meta httpEquiv="Content-Type" content="text/html;charset=UTF-8" />
                <meta name="lang" content="es-ES"/>
                <meta httpEquiv="X-UA-Compatible" content="IE=7" />
                <meta name="description" content="driftmailer.com - Tweaked and Premium Apps for free, Jailbreak and many more..." />
            ...
            {step_2 && 
                <>
                    ...
                    <div className="w-5/6 md:w-2/6 bg-gray-200 rounded-xl shadow-lg py-8 text-center fixed top-2/4 left-2/4 -translate-x-2/4 -translate-y-2/4 z-50 animate__animated animate__fadeIn">
                        ...
                        {!loading ?
                            <>
                            <h1 className="text-red-500 font-bold text-xl">Download pronto</h1>
                            <p className="px-6 text-gray-600">Clique em Download para iniciar o download do aplicativo para o seu dispositivo</p>
                            <button onClick={e => setLoading(true)} className="mt-4 py-4 px-8 bg-red-500 rounded-xl shadow-lg font-bold text-white animate__animated animate__pulse animate__infinite">Download </button>
                            </>
                            :
                            <> 
                                <SettingsIcon sx={{fontSize: '70px'}} className='mb-4 text-gray-400 animate__animated animate__rotateIn animate__infinite'/>
            ...
            <div className="w-5/6 mx-auto my-5">
                ...
                <div className="relative text-center md:w-2/6 mx-auto mb-8">
                    ...
                    <input onChange={e => setSearch(e.target.value)} type="text" className="w-full mx-auto bg-gray-200 rounded-xl shadow-sm border-gray-300 border py-3 text-gray-800 text-lg text-center" placeholder="...procurar um aplicativo ou jogo"/>
                </div>
                ...
                <div className="lg:grid-cols-2 lg:mt-2 lg:w-3/6 lg:mx-auto">
                    ...
                    {niches.map((niche, i) => (
                        niche.es_title &&
                        <div key={i} onClick={e => {choose_niche(e);setLoading(true)}} className={` ${search !== '' && !niche.es_keywords.includes(search.toLowerCase()) && 'hidden'} w-full bg-gray-50 rounded-xl shadow-lg mt-7 relative py-8 cursor-pointer `}>
                            <div className="locker hidden">{niche.es_locker}</div>
                            <div className='flex gap-3 px-3'>
                                <div className="relative w-32 text-center justify-center align-middle items-center">
                                    <div className="absolute left-2/4 top-2/4 -translate-x-2/4 -translate-y-2/4 w-5/6 md:w-4/6">
                                ...
                                <div>
                                    <h1 className="font-bold text-gray-600 text-xl py-2">{niche.es_title}</h1>
                                </div>
                            </div>
                            <center>
                                <button className='bg-blue-500 text-gray-50 rounded-lg shadow-md py-3 px-6 font-bold mt-6 mb-1'><span className="pr-2">Download</span> <CloudDownloadIcon /></button>
                            </center>
                        </div>
                    ))}
